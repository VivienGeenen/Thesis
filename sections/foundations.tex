...
\label{subsection:balancing energy}
    ...
    \end{equation}
    The deduction of the inner energy $U$ \nomenclature[P]{$U$}{inner energy\nomunit{$J$}} starts with the complete differential description of the specific inner energy $du$ as \cite{Cerbe.2021}:
    \begin {equation} 
...
\label{subsection:convection}
    ...
    . There are two possibilities to determine mathematically the heat transfer coefficient. Both require a temperature difference $\Delta T$ and either a temperature gradient $\partial T/\partial x$ \nomenclature[P]{$\partial T/\partial x$}{temperature gradient\nomunit{$K/m$}} or a heat flux $\dot{Q}$.
    \cite{.2013} 
