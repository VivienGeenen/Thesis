...
\label{subsection:balancing energy}
    ...
    \end{equation}
    The deduction of the inner energy $U$ \nomenclature[P]{$U$}{inner energy\nomunit{$J$}} starts with the complete differential description of the specific inner energy $du$ as \cite{Baehr.2016b}:
    \begin {equation} 
    \label{eq:spezificinnerEnergy}
    du = (\frac{\partial u}{\partial T})_\text{v} dT + (\frac{\partial u}{\partial v})_\text{T} dv
    \end{equation}
    The specific volume $dv$ is negligible in buildings, and the specific heat capacity during constant volume has the expression $c_\text{v} = (\frac{\partial u}{\partial T})_\text{v}$ \cite{Baehr.2016b}. After replacing the specific values by volume, we obtain the relation for the inner energy $U$, with the mass $m$. 
        \begin{equation}
...
\label{subsection:conduction}
    ...
    \label{eq:fourier}
        \dot{\textbf{q}} = - \lambda \nabla T
    \end{equation}
    describes the conduction according to Fourier \cite{.2013}. $\lambda$ is \nomenclature[G]{$\lambda$}{thermal conductivity\nomunit{$W/(m K)$}} the thermal conductivity with the assumption of being constant and $\dot{\textbf{q}}$ and $T$ represent the specific heat flux and the temperature. The thermal conductivity is dependent on the material, such as concrete, wood or bricks. 
    \newline
    To know the heat flux $\dot{Q}$, it is necessary to expand the above equation with the area $A$, the thickness of the conductive medium $d$ and a temperature difference $\Delta T$ \nomenclature[P]{$\Delta T$}{temperature difference \nomunit{$K$}} assuming one significant direction of the heat flux $\dot{Q}$ to:
    \begin{equation}
...
\label{subsection:convection}
    ...
    \end{equation}
    with the heat transfer coefficient $\alpha$ \nomenclature[G]{$\alpha$}{heat transfer coefficient\nomunit{$W/(m^2 K)$}}, especially for building modelling the wall temperature $T_w$ and the environment temperature $T_\infty$ \cite{Griesinger.2019}
    . There are two possibilities to determine the heat transfer coefficient. Both require a temperature difference $\Delta T$ and either a temperature gradient $\partial T/\partial x$ \nomenclature[P]{$\partial T/\partial x$}{temperature gradient\nomunit{$K/m$}} or a heat flux $\dot{Q}$.
    \cite{.2013} 
...
\label{electricalanalogy}
    ...
    Similar to an electrical network, the potential is represented by the temperature at one node and the heat flux corresponds to the current. In analogy to the electric domain, we can describe the Ohm's law in heat transfer by:  
     \begin{equation}
    \label{eq:Ohm}
   ...
   %\captionof{figure}[]{}
    ...
    \cite{Kuchling.2007}. 
    Because of the thermal analogy of electrical laws, the current is replaced by heat flux. The following differential equation results for the node  $T_\text{wall}$ using Ohm's law ($\dot{Q}=\Delta T/R$) and the first low of thermodynamics as deduced in \autoref{eq:1HSkurz} with the inner energy from \autoref{eq:innerEnergy}.     
    \begin{equation}
