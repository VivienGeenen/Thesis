...
\label{thermalmodel}
   ...
   % \vspace{0.3cm}
    ...
    At last, we obtain the ready model (see in Appendix the initial values and the identified values).
    ...
    \subsubsection{Training and verification of the thermal model}
    ...
    The Root Mean Square Error (RMSE) and the maximal residual ($\mathrm{R}_\text{max}$) is used as verification measure of the model.\newline
    ...
    \end{equation}
    Here, is $N$ the number of measurements or simulated values. In \autoref{tab:valiresults}, the results of the RMSE and the $\mathrm{R}_\text{max}$ are listed for the two optimized outputs after the training and the verification period.
    \begin{table}[]
        \centering
